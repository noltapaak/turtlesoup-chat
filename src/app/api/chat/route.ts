  ...
  try {
    ...
    if (isGuess) {
      ...
      **게임 답변 규칙:**
      ...
      1.  **의미적으로 사실일 경우 (긍정 답변):** 사용자의 질문이 '정답 정보'와 맥락상 일치하거나 매우 유사하다면, "예, 맞습니다."로 시작하고, 관련된 구체적인 사실을 간결하게 덧붙여 설명해주세요.
          *   *(예시) 정답: "남자는 시각장애인이다." -> 사용자 질문: "남자는 앞을 못 보나요?" -> **좋은 답변:** "예, 맞습니다. 그는 앞을 보지 못하는 상태였습니다."*
          *   *(예시) 정답: "남자는 인육을 먹었다." -> 사용자 질문: "남자가 먹은게 혹시 사람고기인가요?" -> **좋은 답변:** "예, 맞습니다. 그가 먹은 것은 사람의 고기였습니다."*
      ...
      4.  **정답과 직접적인 관련이 없을 경우 (무관 답변):** 질문이 사건의 진상과 전혀 관련이 없을 경우, "그 질문은 현재 사건과 직접적인 관련은 없습니다." 라고 답변해주세요.

      5.  **정답 추측 유도:** 사용자의 질문이 스무고개 질문이 아니라 사건의 전말을 한번에 설명하려는 것처럼 보인다면, "정답을 추측하시려면 '정답 외치기' 버튼을 사용해주세요." 라고 정확히 답변하여 버튼 사용을 유도해야 합니다.

      **추가 응답 스타일 가이드:**
      *   **간결함 유지:** 답변은 명확하고 간결하게 유지하되, 지나치게 단답형이 되지 않도록 합니다.
      *   **유도 금지:** 사실 확인 외에 다음 질문을 제안하거나 추리를 유도하는 발언은 하지 마세요.
