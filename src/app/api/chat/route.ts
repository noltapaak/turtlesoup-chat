...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (isGuess) {
      ...
      // 1. AI에게 의미적/논리적 일치도 판정 요청
      const systemMessageForGuess = `당신은 추리 게임의 최종 판정관입니다. 플레이어의 최종 추리가 '비밀 정답'과 논리적으로 완벽하게 일치하는지 판별해야 합니다.
...
**판정 기준:**
1.  **핵심 요소 포함**: 플레이어의 추리가 비밀 정답의 핵심 요소(인물, 장소, 사건, 동기 등)를 모두 포함하고 있습니까?
2.  **인과관계 및 논리성**: 추리의 내용이 비밀 정답과 **인과관계 및 서사 구조** 면에서 정확히 일치합니까? 단순히 키워드만 나열하는 것이 아니라, '누가, 무엇을, 어떻게, 왜'에 대한 설명이 비밀 정답의 이야기와 같아야 합니다.
3.  **상식적 타당성**: "눈사태를 먹고 죽었다"와 같이, 상식적으로 말이 되지 않거나 비논리적인 부분은 없습니까?

위 세 가지 기준을 **모두** 만족하는 경우에만 'YES'로 판정하세요. 하나라도 만족하지 못하면 'NO'입니다.
...
If no keywords are matched, return an empty array [].`;
    ...
    // 3. AI를 이용한 '판정'
    const systemMessageContent = `당신은 논리적 추리 게임의 심판입니다. 당신의 임무는 플레이어의 질문이 '비밀 정답'과 논리적으로 일치하는지 엄격하게 판정하는 것입니다.
...
**판정 규칙:**
1.  **YES**: 질문의 내용이 비밀 정답에 명시된 사실과 **정확히 일치**하거나, 그 사실로부터 **논리적으로 직접 파생**될 수 있는 경우에만 해당됩니다. 질문 자체가 상식적으로 말이 되어야 합니다.
2.  **NO**: 질문의 내용이 비밀 정답과 **명백히 다르거나, 논리적으로 모순**되거나, "눈사태로 자해했다"와 같이 **상식적으로 말이 되지 않는** 경우에 해당됩니다.
3.  **IRRELEVANT**: 질문의 내용이 정답의 핵심적인 인과관계와 직접적인 관련이 없는 주변 정보일 경우에 해당됩니다.

**매우 중요:** 당신의 답변은 게임의 성패를 좌우합니다. 키워드가 포함되어 있다는 이유만으로 'YES'라고 답해서는 안 됩니다. 질문의 **논리적 타당성**을 최우선으로 검토하세요.
